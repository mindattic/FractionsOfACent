...
- **Non-retention** is enforced at the code level. Python:
  [scraper.py:145-168](scraper.py). C#:
  [v2/Scraper.cs:82-111](v2/Scraper.cs). The raw regex match is bound
...
- **Responsible disclosure**: every finding should be forwarded to the
  ...
  [disclosure.py](disclosure.py)) so the key is revoked. GitHub's push
...
export GITHUB_TOKEN=ghp_xxx           # fine-grained PAT, public-repo read
...
deduplicating by `(key_sha256, file_html_url)`.
